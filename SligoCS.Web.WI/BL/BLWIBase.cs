...
namespace SligoCS.BL.WI
{
    ...
    public class BLWIBase
    {
        ...
        {
            List<int> years = new List<int>(new int[] { _currentYear });
            if (CompareTo.Key == CompareToKeys.Years)
            {
                if (ViewBy.Key == GroupKeys.Disability)
                {
                    //see bug 516, then Bug 670.
                    years = new List<int>(new int[] { startYear, _currentYear });
                }
                else
                {
                    years = new List<int>(new int[] { startYear, _currentYear });
        ...
        protected virtual List<int> GetYearList()
        {
            ...
            if (CompareTo.Key == CompareToKeys.Years)
            {
                ...
                {
                    //see bug 516, then Bug 670.
                    years = new List<int>( new int[] {2003, _currentYear});
                }
                else
                {
                    years = new List<int>( new int[] {_trendStartYear, _currentYear});
                }
            }
        ...
        public int CurrentYear { get { return _currentYear; } set { _currentYear = value; } }
