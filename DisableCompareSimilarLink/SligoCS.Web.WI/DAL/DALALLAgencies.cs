...
namespace SligoCS.DAL.WI
{
    public class DALALLAgencies : DALWIBase
    {
        ...
        public static String GetSchoolBySubstrSQL(string school_substr)
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("SELECT fullkey, DistrictName, CountyName, Schoolname, LowGrade, HighGrade, AgencyType, SchoolType  " + FROM + "  WHERE SchoolName IS NOT NULL AND AgencyTypeLabel = 'School' and WINSS = 1 AND SchoolName LIKE '%{0}%' Order By Schoolname", school_substr);
