...
namespace SligoCS.Tests.WI.NUnitASPTesters
{
    ...
    public class PostGradPageTester : BaseTester
    {
        ...
        public void Bug844_PostGrad_Columns()
        {
            ...
            PostGradPlan plan = new PostGradPlan();
            Group group = new Group();
            group.Value = group.Range[GroupKeys.All];
            plan.Value = plan.Range[PostGradPlanKeys.All];

            //RULE:  Show=all, ViewBy=All Students, CompareToEnum=PriorYears:
            //select PriorYear as ' '
            //, [Number of Graduates]
            //, [% 4-Year College]
            //, [% Voc/Tech College]
            ...
            List<string> cols = postGrad.GetVisibleColumns(group, SligoCS.BL.WI.OrgLevel.School, SligoCS.BL.WI.CompareToEnum.PRIORYEARS, SchoolType.AllTypes, plan);
            ...
            //RULE: Show=4-year college, ViewBy=AllStudents, CompareToEnum=PriorYears:
            //select PriorYear as ' '
            //, [Number of Graduates]
            //, [Number 4-Year College] 
            //, [% 4-Year College] 

            plan.Value = plan.Range[PostGradPlanKeys.FourYr];

            cols = postGrad.GetVisibleColumns(group, SligoCS.BL.WI.OrgLevel.School, SligoCS.BL.WI.CompareToEnum.PRIORYEARS, SchoolType.AllTypes, plan);
        ...
        public void Bug898E_OrgLevelDistrict()
        {
            ...
            List<string> cols = pgp.GetVisibleColumns(group, SligoCS.BL.WI.OrgLevel.District, SligoCS.BL.WI.CompareToEnum.DISTSTATE, SchoolType.AllTypes, plan);
