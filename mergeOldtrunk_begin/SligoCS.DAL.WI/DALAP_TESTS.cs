...
namespace SligoCS.DAL.WI
{
    public class DALAP_TESTS : DALWIBase
    {
        ...
        {             
            ...
            sb.Append(sql.WhereClauseCSV(SQLHelper.WhereClauseJoiner.NONE, "Sex", sexCode));

            //Adds " ... AND (RaceCode in (1, 2, 3, 4, 5)) ..."
            sb.Append(sql.WhereClauseCSV(SQLHelper.WhereClauseJoiner.AND, "Race", raceCode));

            //Adds " ... AND ((GradeCode >= 16) AND (GradeCode <= 64)) ..."
            sb.Append(sql.WhereClauseMinAndMaxInclusive(SQLHelper.WhereClauseJoiner.AND, "Grade", gradeCodeRange));
            ...
            sb.Append(sql.WhereClauseCSV(SQLHelper.WhereClauseJoiner.AND, "ExamCode", examCodes));
            ...
            //order by clause
            //sb.AppendFormat(" ORDER BY {0}", sql.ConvertToCSV_X(orderBy, false));
