...
namespace SligoCS.Web.WI
{
    ...
    ///   
    /// NOTE:  I originally wanted for this class to be in the website SligoCS.Web.WI rather than in this
    /// DLL (SligoCS.Web.Base.dll).  At the time I wrote the first code for this class, the Wisconsin website
    /// was still a website project as opposed to a website application.  A website project cannot contain
    /// a standalone class, any common code must be added in a folder named "AppCode".  At the time I felt
    /// it was better to add the class to the DLL than put it in the AppCode folder.  
    /// 
    /// In the last few weeks (approx mid-October 2007) we upgraded from a website project to a website application.
    /// I am now moving the ParamsHelper class back to the place I originally wanted it to reside- the website.
    /// --djw 11/25/07
    /// </summary>
    [Serializable]
    public class ParamsHelper
    {
        ...
        private ViewByGroup viewBy;

        private static ParamsHelperXXX defaultValues = null;
        #endregion
        ...
        public void LoadFromContext()
        {
            ...
            foreach (PropertyInfo property in properties)
            {
                ...
                if (name != ITEM)
                {
                    ...
                    string sessionVarName = string.Format("{0}.{1}", SESSIONKEY_PARAMSHELPER, name);
                    object ovalue = null;
                    if ((queryString != null) && (queryString[name] != null))
                    {
                        ovalue = queryString[name];
                    }
        ...
        /// </summary>
        private ParamsHelperXXX DefaultValues
        {
            get
            {
                if (defaultValues == null)
                {
                    ...
                    if ((HttpContext.Current != null) && (HttpContext.Current.Server != null))
                    {
                        ...
                        defaultValues = (ParamsHelperXXX)ser.Deserialize(reader);
                        reader.Close();                        
                    }                   
                }
