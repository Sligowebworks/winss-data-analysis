...
namespace SligoCS.Web.WI.WebSupportingClasses
{
    public class WinssDataGrid : GridView
    {
            ...
            : base()
        {
            ...
            if (Width.IsEmpty) Width = Unit.Pixel(460);
        ...
        protected virtual void SortingHandler(Object sender, GridViewSortEventArgs args)
        {
            //System.Data.DataView dv = ((System.Data.DataSet)DataSource).Tables[0].DefaultView;
            //dv.Sort = 
            ((System.Data.DataView)DataSource).Sort = args.SortExpression + " " + ((args.SortDirection == SortDirection.Ascending) ? "ASC" : "DESC");
        }
        /// <summary>
        ...
        /// <summary>
        /// Search the Columns Collection for the given databinding fieldname.
        /// </summary>
        /// <param name="DataFieldName"></param>
        /// <returns></returns>
        public int FindBoundColumnIndex(string DataFieldName)
        {
            int x = -1;
            foreach (DataControlField col in Columns)
            {
                ...
                {// case insensitive match
                    if (((BoundField)col).DataField.ToLower() == DataFieldName.ToLower())  break;
                }
            }
            return x;
        }
        ...
        {
            int colIndex = FindBoundColumnIndex(DataFieldName);
            // hct:  should have code for catching exception here?? like colIndex = -1 ect
            // mzd:  would be nice to have error logging here to detect DataField not found by FindBoundColumnIndex()
            if (colIndex >= 0)
            {
                Columns[colIndex].Visible = visible;
            }
            else { throw new Exception(colIndex.ToString()); }
        }
        /// <summary>
        ...
        {
            GridViewSortEventArgs args = new GridViewSortEventArgs("Year", SortDirection.Descending);
            OnSorting(args);
